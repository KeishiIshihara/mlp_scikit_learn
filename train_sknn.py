...
for i in range(num_of_trials): # draw all result with light color
    ax.plot(units, scores_train[i], color='blue', linestyle='--', linewidth=0.2)
    ax.plot(units, scores_test[i],color='orange', linestyle='--', linewidth=0.3)
...
ax.grid(which='both')
ax.set_title('Num of Neurons (train:test={}:{}, Trials={})'.format(1.-test_size,test_size,num_of_trials))
